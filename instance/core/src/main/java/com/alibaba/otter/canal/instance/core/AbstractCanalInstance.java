...
public class AbstractCanalInstance extends AbstractCanalLifeCycle implements CanalInstance {
    ...
    private static final Logger                      logger = LoggerFactory
        .getLogger(AbstractCanalInstance.class);
    ...
    public boolean subscribeChange(ClientIdentity identity) {
        if (StringUtils.isNotEmpty(identity.getFilter())) {
            ...
            if (isGroup) {
                // 处理group的模式
                List<CanalEventParser> eventParsers = ((GroupEventParser) eventParser)
                    .getEventParsers();
    ...
    protected void startEventParserInternal(CanalEventParser eventParser, boolean isGroup) {
        if (eventParser instanceof AbstractEventParser) {
            ...
            // 首先启动log position管理器
            CanalLogPositionManager logPositionManager = abstractEventParser
