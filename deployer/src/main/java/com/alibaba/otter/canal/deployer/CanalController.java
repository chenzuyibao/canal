...
public class CanalController {
    ...
    private static final Logger                      logger   = LoggerFactory
        .getLogger(CanalController.class);
    private Long                                     cid;
    private String                                   ip;
    private int                                      port;
    // 默认使用spring的方式载入
    ...
    public CanalController(final Properties properties) {
        ...
        try {
            int metricsPort = Integer
                .valueOf(getProperty(properties, CanalConstants.CANAL_METRICS_PULL_PORT));
        ...
        ServerRunningMonitors.setRunningMonitors(
            MigrateMap.makeComputingMap(new Function<String, ServerRunningMonitor>() {
                ...
                public ServerRunningMonitor apply(final String destination) {
                    ...
                    runningMonitor.setListener(new ServerRunningListener() {
                        ...
                        public void processActiveEnter() {
                            try {
                                MDC.put(CanalConstants.MDC_DESTINATION,
                        ...
                        public void processStart() {
                            try {
                                if (zkclientx != null) {
                                    final String path = ZookeeperPathUtils
        ...
        if (autoScan) {
            defaultAction = new InstanceAction() {
                ...
                public void start(String destination) {
                    ...
                    if (!embededCanalServer.isStart(destination)) {
                        // HA机制启动
                        ServerRunningMonitor runningMonitor = ServerRunningMonitors
                            .getRunningMonitor(destination);
            ...
            instanceConfigMonitors = MigrateMap
                .makeComputingMap(new Function<InstanceMode, InstanceConfigMonitor>() {
                    
                    public InstanceConfigMonitor apply(InstanceMode mode) {
                        int scanInterval = Integer.valueOf(
                            getProperty(properties, CanalConstants.CANAL_AUTO_SCAN_INTERVAL));
        ...
        instanceGenerator = new CanalInstanceGenerator() {
            ...
            public CanalInstance generate(String destination) {
                ...
                if (config.getMode().isManager()) {
                    ...
                    instanceGenerator
                        .setCanalConfigClient(managerClients.get(config.getManagerAddress()));
    ...
    private void initInstanceConfig(Properties properties) {
        ...
        for (String destination : destinations) {
            ...
            if (oldConfig != null) {
                logger.warn("destination:{} old config:{} has replace by new config:{}",
    ...
    private InstanceConfig parseInstanceConfig(Properties properties, String destination) {
        ...
        if (config.getMode().isManager()) {
            ...
            String springXml = getProperty(properties,
                CanalConstants.getInstancSpringXmlKey(destination));
            if (StringUtils.isNotEmpty(springXml)) {
